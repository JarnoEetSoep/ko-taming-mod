...
public abstract class MixinMuleEntity extends LivingEntity implements TorporEntity, KOTamableEntity {

    private int torpor = 0;
    private final int BASETORPOR = Main.getBaseTorporConfig().baseMuleTorpor;
    private int tamingPercent = 0;
    private PlayerEntity owner = null;
    private PlayerEntity knockedOutBy;

    protected MixinMuleEntity(EntityType<? extends LivingEntity> entityType, World world) {
    ...
    public ArrayList<Map<Item, Integer>> getTamingFood() {
        ...
        tamingFood.get(0).put(Items.PUMPKIN_PIE, Math.round(40 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(1).put(Items.WHEAT, Math.round(32 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(2).put(Items.BEETROOT, Math.round(8 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
