...
public abstract class MixinFoxEntity extends LivingEntity implements TorporEntity, KOTamableEntity {

    private int torpor = 0;
    private final int BASETORPOR = Main.getBaseTorporConfig().baseFoxTorpor;
    private int tamingPercent = 0;
    private PlayerEntity owner = null;
    private PlayerEntity knockedOutBy;

    protected MixinFoxEntity(EntityType<? extends LivingEntity> entityType, World world) {
    ...
    public ArrayList<Map<Item, Integer>> getTamingFood() {
        ...
        tamingFood.get(0).put(Items.CHICKEN, Math.round(65 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(1).put(Items.MUTTON, Math.round(50 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(2).put(Items.COOKED_CHICKEN, Math.round(32 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(2).put(Items.COOKED_MUTTON, Math.round(32 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(2).put(Items.SWEET_BERRIES, Math.round(20 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
