...
public abstract class MixinPolarBearEntity extends LivingEntity implements TorporEntity, KOTamableEntity {
    ...
    public ArrayList<Map<Item, Integer>> getTamingFood() {
        ...
        tamingFood.get(0).put(Items.SALMON, Math.round(25 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(1).put(Items.COD, Math.round(20 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(2).put(Items.TROPICAL_FISH, Math.round(10 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(3).put(Items.PUFFERFISH, Math.round(3 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
