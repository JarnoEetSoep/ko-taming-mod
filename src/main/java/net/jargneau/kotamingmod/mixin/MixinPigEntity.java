...
public abstract class MixinPigEntity extends LivingEntity implements TorporEntity, KOTamableEntity {

    private int torpor = 0;
    private final int BASETORPOR = Main.getBaseTorporConfig().basePigTorpor;
    private int tamingPercent = 0;
    private PlayerEntity owner = null;
    private PlayerEntity knockedOutBy;

    protected MixinPigEntity(EntityType<? extends LivingEntity> entityType, World world) {
    ...
    public ArrayList<Map<Item, Integer>> getTamingFood() {
        ...
        tamingFood.get(0).put(Items.POTATO, Math.round(55 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(1).put(Items.CARROT, Math.round(50 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(2).put(Items.BEETROOT, Math.round(40 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
        tamingFood.get(3).put(Items.WHEAT, Math.round(30 * Main.getGeneralConfiguration().tamingSpeedMultiplier));
